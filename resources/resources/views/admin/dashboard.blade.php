...
<div class="stats-grid">
    ...
    <div class="stat-card">
        ...
        <div class="stat-value">{{ $stats['missions_en_attente'] }}</div>
    ...
    <div class="stat-card">
        ...
        <div class="stat-value">{{ $stats['missions_en_pause'] }}</div>
        <div class="stat-label">En Pause</div>
    </div>
    <div class="stat-card">
        <div class="stat-icon red">⛔</div>
        <div class="stat-value">{{ $stats['missions_suspendues'] }}</div>
        <div class="stat-label">Suspendues</div>
    </div>
    <div class="stat-card">
        <div class="stat-icon green">👨‍🔧</div>
        <div class="stat-value">{{ $stats['techniciens_disponibles'] }}<span style="font-size:0.9rem;color:var(--text-muted)">/{{ $stats['total_techniciens'] }}</span></div>
        ...
        <div class="table-responsive">
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <a href="{{ route('admin.missions.show', $mission) }}" style="color: var(--text-primary); font-weight: 600;">
                                {{ Str::limit($mission->titre, 30) }}
                            </a>
                        </td>
                        ...
                        <td>
                            @if($mission->chefEquipe)
                                <span style="font-size:0.8rem">👑 {{ $mission->chefEquipe->name }}</span>
                            @endif
                            @if($mission->techniciens->count() > 1)
                                <span class="text-muted" style="font-size:0.75rem">+{{ $mission->techniciens->count() - 1 }}</span>
                            @endif
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="5" class="text-center text-muted" style="padding:30px">Aucune mission pour le moment</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="card-body" style="padding: 12px;">
            ...
            <div class="team-member">
                ...
                <div style="flex:1; min-width:0;">
                    <div style="font-weight:600; font-size:0.88rem;">{{ $tech->name }}</div>
                    <div style="font-size:0.75rem; color:var(--text-muted);">{{ $tech->missions_count }} mission(s)</div>
                </div>
