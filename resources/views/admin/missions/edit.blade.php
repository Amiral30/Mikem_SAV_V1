...
<div style="max-width:800px;">
    <div class="card">
        <div class="card-header"><h3>✏️ Modifier</h3><span class="badge {{ $mission->statut_class }}">{{ $mission->statut_label }}</span></div>
        <div class="card-body">
            ...
            <form action="{{ route('admin.missions.update', $mission) }}" method="POST">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                    ...
                    <div class="form-group"><label for="date_mission">Date *</label><input type="date" id="date_mission" name="date_mission" class="form-control" value="{{ old('date_mission', $mission->date_mission->format('Y-m-d')) }}" required></div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div style="max-height:250px;overflow-y:auto;background:var(--bg-input);border:1px solid var(--border-color);border-radius:var(--radius-sm);padding:12px;">
                        ...
                        <div class="form-check">
                            <input type="checkbox" name="techniciens[]" value="{{ $tech->id }}" id="tech_{{ $tech->id }}" {{ in_array($tech->id, old('techniciens', $assignedIds))?'checked':'' }} onchange="updateChefOptions()">
...
function updateChefOptions() {
    ...
    if (checked.length > 1) {
        ...
        checked.forEach(cb => { const t = techniciens.find(x => x.id == cb.value); if(t) { const sel = t.id == currentChefId ? 'selected' : ''; chefSelect.innerHTML += `<option value="${t.id}" ${sel}>${t.name}</option>`; }});
