...
<div class="btn-group">
    ...
    <form action="{{ route('admin.missions.destroy', $mission) }}" method="POST" onsubmit="return confirm('Supprimer ?')">@csrf @method('DELETE')<button class="btn btn-danger btn-sm">🗑️ Supprimer</button></form>
</div>
@endsection
...
<div class="mission-detail-grid">
    <div>
        ...
        <div class="card">
            <div class="card-header"><h3>📝 Rapports</h3></div>
            <div class="card-body">
    ...
    <div>
        <div class="card">
            <div class="card-header"><h3>👨‍🔧 Équipe</h3><span class="badge badge-info">{{ $mission->is_groupe?'Groupe':'Individuel' }}</span></div>
            <div class="card-body" style="padding:12px;">
                <ul class="team-list">
                    @forelse($mission->techniciens as $tech)
                    <li class="team-member">
                        <div class="member-avatar">{{ strtoupper(substr($tech->name, 0, 2)) }}</div>
                        <div style="flex:1;"><div style="font-weight:600;font-size:0.88rem;">{{ $tech->name }} @if($tech->pivot->is_chef_equipe)<span class="chef-badge">👑 Chef</span>@endif</div><div style="font-size:0.75rem;color:var(--text-muted);">{{ $tech->email }}</div></div>
